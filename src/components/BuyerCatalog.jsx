...
export default function BuyerCatalog() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 pb-16">
      ...
      <section className="relative overflow-hidden rounded-2xl sm:rounded-3xl bg-slate-900 border border-slate-800 p-4 sm:p-8 lg:p-10 shadow-2xl min-h-[260px] sm:min-h-[380px] lg:min-h-[420px] flex items-center w-full">
        ...
        {HERO_SLIDER_IMAGES.map((imgItem, idx) => (
          <div 
            ...
            className={`absolute inset-0 w-full h-[125%] -top-[10%] pointer-events-none bg-cover bg-center transition-all duration-1000 ease-in-out ${
              idx === heroBgIndex ? 'opacity-90 scale-105' : 'opacity-0 scale-100'
            }`}
            style={{
        ...
        <div className="absolute bottom-3 right-4 z-20 flex items-center gap-1.5 bg-slate-950/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/20 shadow-lg">
          ...
          {HERO_SLIDER_IMAGES.map((_, idx) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 cursor-pointer ${
                idx === heroBgIndex ? 'w-6 bg-emerald-400' : 'w-2 bg-white/40 hover:bg-white/70'
              }`}
              title={`Slide ${idx + 1}`}
            />
          ))}
        ...
        {/* Hero Content */}
        <div className="relative z-10 max-w-3xl space-y-2 sm:space-y-4 text-white">
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-indigo-600/50 text-indigo-100 text-[10px] sm:text-xs font-extrabold border border-indigo-400/40 backdrop-blur-md shadow-md">
              <Package className="w-3.5 h-3.5 text-indigo-300" />
              <span>{approvedProducts.length}+ Direct Products</span>
            </span>

            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-600/50 text-emerald-100 text-[10px] sm:text-xs font-extrabold border border-emerald-400/40 backdrop-blur-md shadow-md">
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-300" />
              <span>Verified Factories & Wholesalers</span>
            </span>
          </div>

          <h1 className="text-xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight leading-tight font-serif drop-shadow-md">
            Direct Wholesale & <span className="text-emerald-400">Nepal Sourcing Hub</span>
          </h1>

          <p className="text-slate-200 text-xs sm:text-base leading-relaxed font-medium drop-shadow line-clamp-2 sm:line-clamp-none">
          ...
          <div className="pt-1 flex items-center gap-3">
            <button
              ...
              className="px-4 py-2.5 sm:px-6 sm:py-3.5 rounded-xl sm:rounded-2xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 shadow-xl shadow-emerald-950/60 font-extrabold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all hover:scale-105 active:scale-95 cursor-pointer"
            >
            ...
            <button
              ...
              className="px-4 py-2.5 sm:px-5 sm:py-3 rounded-xl sm:rounded-2xl bg-white/10 hover:bg-white/20 text-white font-bold text-xs sm:text-sm border border-white/20 backdrop-blur-md transition-all cursor-pointer"
            >
